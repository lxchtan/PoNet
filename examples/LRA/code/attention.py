...
class Attention(nn.Module):
  def __init__(self, config):
    ...
    elif self.attn_type.startswith("fnet"):
      from attention_fnet import FFTAttention
      self.attn = FFTAttention(config)
    elif self.attn_type.startswith("ponet"):
      from attention_ponet import PoNetAttention
      self.attn = PoNetAttention(config)
      self.W_local = nn.Linear(self.dim, self.dim)
      self.W_segment = nn.Linear(self.dim, self.dim)
    ...
    self.sp_ff = nn.Identity()

  def forward(self, X, mask):

    if self.attn_type.startswith("longformer") or self.attn_type.startswith("reformer") or self.attn_type.startswith("fnet"):
      with torch.cuda.amp.autocast(enabled=False):
        attn_out = self.attn(X.float(), mask.float())
    elif self.attn_type.startswith("ponet"):
      Q = self.split_heads(self.W_q(X))
      K = self.split_heads(self.W_k(X))
      O = self.split_heads(self.W_v(X))

      local = self.W_local(X)
      segment = self.W_segment(X)
      with torch.cuda.amp.autocast(enabled=False):
        attn_out = self.attn(X.float(), Q.float(), K.float(), O.float(), local.float(), segment.float(), mask.float())
    else:
      Q = self.split_heads(self.W_q(X))
      K = self.split_heads(self.W_k(X))
    ...
    if self.attn_type.startswith("fnet"):
      self.ff = self.sp_ff
