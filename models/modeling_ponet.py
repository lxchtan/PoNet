...
from transformers.models.bert.configuration_bert import BertConfig as PoNetConfig
PoNetConfig.model_type = 'ponet'
from transformers.models.bert.modeling_bert import (
  BertEmbeddings as PoNetEmbeddings,
  BertSelfOutput as PoNetSelfOutput,
  BertOutput as PoNetOutput,
  BertIntermediate as PoNetIntermediate,
  BertPooler as PoNetPooler,
  load_tf_weights_in_bert as load_tf_weights_in_ponet,
  BertLMPredictionHead as PoNetLMPredictionHead
)
...
def segment_max(src, index, dim=1):
  out, _ = scatter_max(src.to(dtype=torch.float32), index, dim=dim)
...
class PoNetSelfAttention(nn.Module):
  ...
  def forward(
    ...
    if self.clsgsepg:
      ...
      context_layer_local = get_win_max(context_layer_local.masked_fill(token_type_mask.unsqueeze(dim=-1), -10000))
...
PONET_START_DOCSTRING = r"""
    ...
    Parameters:
        config (:class:`~transformers.PoNetConfig`): Model configuration class with all the parameters of the model.
...
PONET_INPUTS_DOCSTRING = r"""
    Args:
        input_ids (:obj:`torch.LongTensor` of shape :obj:`({0})`):
            ...
            Indices can be obtained using :class:`~transformers.PoNetTokenizer`. See
